...
{
    public partial class ResturantLogin : Form, IResturantLogin
    {
        public ResturantLogin()
        {
            InitializeComponent();
        ...
        public void blogin_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT * FROM RESTURANT_OWNER WHERE USERNAME = @username COLLATE Latin1_General_BIN AND PASSWORD = @password COLLATE Latin1_General_BIN AND ACCOUNT_STATUS=@status", con);
            ...
            {
                //get resturant name
                cmd = new SqlCommand("SELECT RESTURANT_NAME FROM RESTURANT_OWNER WHERE USERNAME = @username", con);
                cmd.Parameters.AddWithValue("@username", username);
                string resturantname = cmd.ExecuteScalar().ToString();
                ...
                this.Visible = false;
                ResturantHome x = new ResturantHome(resturantname);
                x.Visible = true;

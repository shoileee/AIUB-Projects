...
#define forwardPin 4 //simulation button for direction control
#define backwardPin 5 //simulation button for direction control
#define leftPin 6 //simulation button for direction control
#define rightPin 7 //simulation button for direction control

const int transmit_pin = 12; //simulation transmitter pin
...
const float bendResistance = 100000.0; //change this value according to sensor value when it is in 90 degree bend position
...
const int maxErrorCount = 3; // this variable defines maximum failed attempt of data transmission
...
void setup() {
  ...
  */
  ...
  lcd.display();
  delay(100);
  Serial.println("message = Xaxis,Yaxis,flexSensor,ClawInterrupt,ModeInterrupt---");
}
...
void loop() {
    ...
    else if(digitalRead(backwardPin)==0){
      sendData.yAxis = -255;
    }
...
{
  message = message + d.xAxis +"," + d.yAxis + "," + d.felxSensorClawValue + ","+ d.clawInterrputValue + "," + d.modeInterruptValue;
  mlength = message.length(); // find the number of characters in a message.
  str = message;
  str.toCharArray(cstr,100);
  bool status = vw_send((uint8_t *)cstr, mlength); // 
  vw_wait_tx(); // Wait until the whole message is gone
  if(status){
    Serial.print("Transfered message: ");

...
bool allStage = false;
bool winner = false;
bool isStage1Text= true;
bool isStage2Text= true;
bool isStage3Text= true;
...
void update(int value) {
    //Showing Character x and Y value for placing objects correctly
    cout<<"Character X= "<<character.getX()<<" Character Y= "<<character.getY()<<endl;
...
//Stage text Viewer
void stageTextViewer(int value) {
    if(isStage1Text && isStage1){
        isStage1Text = false;
    }
    else if(isStage2Text && isStage2){
        isStage2Text = false;
    }
    else if(isStage3Text && isStage3){
        isStage3Text = false;
    }
}
...
void display() {
    ...
    if(lives >0){
        ...
        if(isStage1){
            ...
            drawStage01();
            if(isStage1Text){
                drawText("<--------******Stage 1******-------->", -200.0, 400.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 0, 0);
                glutTimerFunc(3000, stageTextViewer, 0);
            }
        }
            ...
            drawStage02();
            if(isStage2Text){
                drawText("<--------******Stage 2******-------->", -200.0, 400.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 0, 0);
                glutTimerFunc(3000, stageTextViewer, 0);
            }
        }
        else if(isStage3){
            ...
            drawStage03();
            if(isStage3Text){
                drawText("<--------******Stage 3******-------->", -200.0, 400.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 0, 0);
                glutTimerFunc(3000, stageTextViewer, 0);
            }
        }
        else if(winner){
            ...
            drawText("Congratulation! You Won" , 7100, 400.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 1, 1);
            drawText("Score: " + to_string(score), 7100, 360.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 0, 0);
            glFlush(); // Render now
        ...
        glColor3f(1.0,0.0,0.0);
        drawText("Game Over" , deadPosition, 400.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 1, 1);
        drawText("Score: " + to_string(score), deadPosition, 360.0f, GLUT_BITMAP_TIMES_ROMAN_24, 1, 0, 0);
        glFlush(); // Render now

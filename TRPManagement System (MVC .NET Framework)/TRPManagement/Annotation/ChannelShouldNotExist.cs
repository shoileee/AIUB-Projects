using System;
using System.Collections.Generic;
using System.ComponentModel.DataAnnotations;
using System.Linq;
using System.Web;
using TRPManagement.Entity_Framework;

namespace TRPManagement.Annotation
{
    [AttributeUsage(AttributeTargets.Property |
AttributeTargets.Field, AllowMultiple = true)]
    public class ChannelShouldNotExist:ValidationAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            var _channelName = value as string;

            // Accessing database and cehking
            TRP_ManagementEntities db = new TRP_ManagementEntities();

            var exists = (from data in db.Channels
                          where data.ChannelName == _channelName
                          select data).SingleOrDefault();
            if (exists != null)
            {
                return new ValidationResult("Channel Name Already Exist");
            }
            return ValidationResult.Success;

        }
    }
}